    ...
    def create_logger():
        class Logger():
            def __init__(self) -> None: 
                f = lambda *x: print("GRAPH WIDGET: ", *x)  # change messenger in whatever script you are importing
...
def log(message, level="info"):
    getattr(logger, level)("GRAPH WIDGET: {}".format(message))  # change RECORDER SAVER IN CLASS NAME
...
class Graph(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # VARS:
        # NB: Dont move these to class vars: will cause error that all
        #     graphs plot the same.
        ...
        self.plot_data = None               # np struct array with plot data, first dim is x next are y(s)
        # default settings
        self.settings = {'linecolor': MO,   # Color of line,
    ...
    def update_ax(self, *args):
        self.ax.points = [self.pos[0], (self.pos[1] + self.size[1]),
    ...
    def do_y_zoom(self):
        ...
        if zoom > 0:
            ...
            low, mid, high = np.nanpercentile(data, 
                                              (5, 50, 95),
                                               method='linear')   # was lower, linear would be better for ourliers but slower
    ...
    def drawData(self, *args):
        ...
        for i, par_name in enumerate(self.plot_data.dtype.names):
            if i == 0:
                ...
                x, minmax[:2] = self.prepareData(x,
                                                 dim[:2],
                                                 min_dat=self.scale['minx'],
                                                 max_dat=self.scale['maxx'])
                ...
                y, minmax[2:] = self.prepareData(y, 
                                                 dim[2:],
                                                 min_dat=self.scale['miny'],
                                                 max_dat=self.scale['maxy'])
                if i == 1:
                    self.scale = {k:v for k, v in 
            ...
            pdat = self.combine_xy_data(x[mask][::step], 
                                        y[mask][::step])
        ...
        for i_label in range(n_labels):
            ...
            if len(self.x_lbl) <= i_label: 
                # create x label
                self.x_lbl.append(
                    GrAx_X_Lbl(size_hint = (1/self.steps, 0.04),
                               pos_hint={'x': i_label / self.steps - (0.05 * (10/self.steps)), 
                                         'y': -0.09},))
            ...
            if len(self.y_lbl) <= i_label:
                # create y label
                self.y_lbl.append(
                    GrAx_Y_Lbl(size_hint = (0.04, 1/self.steps),
                               pos_hint={'x': -0.07, 
                                         'y': 0.97 * (i_label / self.steps) - (0.03 * (10/self.steps))},))
...
<Scr>:
    ...
    class Scr(FloatLayout):
        ...
        def gr_data(self, *args, **kwargs):
            t = time.time()
            n = 100
            data = np.empty(n, dtype=[('time', 'f8')] + [(str(i), 'f4') for i in range(5)])
            f = np.random.choice([lambda i, *x: (1/i) * np.linspace (*x), 
