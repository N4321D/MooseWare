...
class GasSensor : public I2CSensor
{
    ...
    protocol pack(uint8_t *pBuffer, uint8_t) //Compresses modified aspects of the array to the protocol struct
    {
        protocol _protocol;
        _protocol.head = 0xff;
        _protocol.addr = 0x01;
        memcpy(_protocol, pBuf, len);
    ...
    void init()
    {
        changeAcquireMode(true); //Sets chip to active sampling
    ...
    void sample()
    {
        sampled_data[0] = readTepC();
        sampled_data[1] = readGasConc(sampled_data[0]);
    ...
    bool changeAcquireMode(bool active)
    {
        ...
        else {return false} //validates whether the mode change was successful   
    }
